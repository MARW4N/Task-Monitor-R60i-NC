...
# Remove obsolete reference assemblies package if present from previous runs
$oldRefAssemblies = Join-Path $packagesDir "Microsoft.NETFramework.ReferenceAssemblies.net472.1.0.3"
if (Test-Path $oldRefAssemblies) {
    Remove-Item -Path $oldRefAssemblies -Recurse -Force -ErrorAction SilentlyContinue
}
