    ...
    public partial class OptionForm : Form
    {
        ...
        public OptionForm(Options opt, GraphTheme theme, Version version, TaskbarManager manager/*, SystemWatcherControl originalControl*/)
        {
                ...
                AvailableGraphTypes = new Dictionary<string, IList<Counters.ICounter.CounterType>>
            {
                ...
                {"SOUNDCORE",  new List<TaskbarMonitor.Counters.ICounter.CounterType>
                {
                    TaskbarMonitor.Counters.ICounter.CounterType.SINGLE,
                    TaskbarMonitor.Counters.ICounter.CounterType.STACKED,
                    TaskbarMonitor.Counters.ICounter.CounterType.MIRRORED
        ...
        private void btnBrowseSoundcore_Click(object sender, EventArgs e)
        {
            using (var ofd = new OpenFileDialog { Filter = "Executable files (*.exe)|*.exe|All files (*.*)|*.*", Title = "Locate openscq30.exe" })
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    if (this.txtSoundcoreExePath != null)
                        this.txtSoundcoreExePath.Text = ofd.FileName;
                    if (this.Options.Soundcore != null)
                        this.Options.Soundcore.ExePath = ofd.FileName;
                    UpdatePreview();
                }
            }
        }

        private void btnTestSoundcore_Click(object sender, EventArgs e)
        {
            try
            {
                string exePath = this.txtSoundcoreExePath != null ? this.txtSoundcoreExePath.Text.Trim() : (this.Options.Soundcore?.ExePath ?? "");
                string mac = this.txtSoundcoreMac != null ? this.txtSoundcoreMac.Text.Trim() : (this.Options.Soundcore?.MacAddress ?? "");

                if (string.IsNullOrEmpty(exePath) || !System.IO.File.Exists(exePath))
                {
                    MessageBox.Show("Please select a valid openscq30.exe path first.", "Soundcore Test", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                {
                    FileName = exePath,
                    Arguments = $"device --mac-address {mac} setting --get batteryLevelLeft --get batteryLevelRight --get caseBatteryLevel --json",
                    CreateNoWindow = true,
                ...
                using (var proc = System.Diagnostics.Process.Start(psi))
                {
                    ...
                    {
                        MessageBox.Show($"Connection successful!\nOutput:\n{output}", "Soundcore Battery Test", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("No output received from openscq30.exe. Ensure earbuds are connected via Bluetooth.", "Soundcore Battery Test", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Test failed: {ex.Message}", "Soundcore Battery Test", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
